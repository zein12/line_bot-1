...
class Tebakkode_m extends CI_Model {
  ...
  {
    $this->db->set('signature', $signature)->set('events', $body)->insert('eventlog');
...

{
...

  {
    ...
    $data = $this->db->where('number', $questionNum)->get('questions')->row_array();
  ...
  function setUserProgress($user_id, $newNumber)
  {
    ...
    $this->db->set('number', $newNumber)->where('user_id', $user_id)->update('users');
  ...
  function setScore($user_id, $score)
  {
    ...
    $this->db->set('score', $score)->where('user_id', $user_id)->update('users');
